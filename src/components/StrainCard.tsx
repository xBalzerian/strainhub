...
function StrainImg({
  ...
  if (!valid) {
    return (
      <div className={`w-full h-full ${t.bg} flex flex-col items-center justify-center gap-2`}>
        <span className="text-5xl">{emoji}</span>
        <span className={`text-[10px] font-black ${t.text} uppercase tracking-widest`}>
          {strain.type}
        </span>
      </div>
    );
  ...
  return (
    <div className={`relative w-full h-full ${t.bg}`}>
      <Image
  ...
  return (
    <Link href={`/strains/${strain.slug}`} className="group block h-full">
      <article
        ...
        "
      >
        {/* ── IMAGE — top, square, full illustration ──────────── */}
        <div className="relative w-full aspect-square border-b-2 border-black">
          ...
          <StrainImg strain={strain} priority={priority} />
        </div>
            ...
            {strain.flavors?.length > 0 && (
              <div className="flex gap-1">
                {strain.flavors.slice(0, 2).map((f) => (
                  <span key={f} className="text-[9px] px-1.5 py-0.5 bg-gray-50 border border-gray-200 rounded text-gray-500 font-medium">
                    {f}
                  </span>
                ))}
              </div>
