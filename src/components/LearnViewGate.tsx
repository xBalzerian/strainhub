...
export default function LearnViewGate({ children, title }: { children: React.ReactNode; title?: string }) {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      ...
      if (!ok) { setAllowed(false); setShowPaywall(true); }
      setChecked(true);
    };
    check();
  }, [loading]);
    ...
    return (
      <div className="min-h-screen bg-[#f8f8f6] flex items-center justify-center p-6">
        <div className="text-center max-w-sm">
          ...
          <p className="text-gray-500 mb-6">{!user ? "You've read 10 Learn pages today. Sign up free to keep reading." : "Upgrade to Pro for unlimited access to the entire Learn Hub."}</p>
