...
export default function ChatPage() {
  ...
  const { isPro, canChat, chatsRemaining, trackChat, user, profile } = useAuth();
  ...
  async function sendMessage(text?: string) {
    ...
    try {
      ...
      else setError("Failed to get a response. Please try again.");
    } catch {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-64px)] bg-[#F8F8F0]">
      ...
      <div className="bg-white border-b-2 border-black px-6 py-4 flex items-center gap-4">
        <div className="w-10 h-10 bg-lime border-2 border-black rounded-xl flex items-center justify-center text-xl font-black shadow-brutal-sm">🤖</div>
        <div>
        ...
        <div className="ml-auto flex items-center gap-3">
          {isPro ? (
            ...
            <span className="text-xs font-bold text-gray-400">
              ...
              <Link href="/account" className="text-brand underline">Go Pro</Link>
            </span>
          ) : (
            <Link href="/login?redirect=/chat" className="text-xs font-bold bg-lime border-2 border-black px-3 py-1.5 rounded-lg shadow-brutal-sm hover:shadow-brutal hover:-translate-y-0.5 transition-all">
              Sign in to chat →
            </Link>
          )}
          {messages.length > 0 && (
            <button onClick={() => setMessages([])} className="text-xs text-gray-400 hover:text-brand px-3 py-1 border border-gray-200 rounded-lg transition-all">
              Clear
            </button>
          )}
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-4">
        {messages.length === 0 && (
          <div className="max-w-2xl mx-auto">
            <div className="text-center mb-8">
              ...
              {!user && (
                <div className="mt-4 inline-flex items-center gap-2 bg-lime-pale border-2 border-black rounded-xl px-4 py-2 text-sm font-bold">
                  💡 <Link href="/login?redirect=/chat" className="underline">Sign in</Link> to track your usage &amp; go Pro
                </div>
              )}
            </div>
        ...
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"} max-w-3xl mx-auto w-full`}>
            {msg.role === "assistant" && (
              <div className="w-8 h-8 bg-lime border-2 border-black rounded-lg flex items-center justify-center text-sm mr-2 mt-0.5 flex-shrink-0">🤖</div>
            )}
        ...
        {loading && (
          <div className="flex justify-start max-w-3xl mx-auto w-full">
            <div className="w-8 h-8 bg-lime border-2 border-black rounded-lg flex items-center justify-center text-sm mr-2 flex-shrink-0">🤖</div>
        ...
        {error && (
          <div className="max-w-3xl mx-auto w-full">
            <div className="bg-red-50 border-2 border-red-200 rounded-2xl px-4 py-3 text-sm text-red-600 font-medium flex items-center justify-between gap-4">
              <span>{error}</span>
              {!isPro && <Link href="/account" className="flex-shrink-0 bg-lime border-2 border-black rounded-lg px-3 py-1 text-brand text-xs font-black shadow-brutal-sm hover:shadow-brutal hover:-translate-y-0.5 transition-all">Get Pro →</Link>}
            </div>
          </div>
        )}
        <div ref={bottomRef} />
      ...
      <div className="bg-white border-t-2 border-black px-4 py-4">
        <div className="max-w-3xl mx-auto flex gap-3 items-end">
          <textarea
            ...
            disabled={loading}
            rows={1}
            className="flex-1 px-4 py-3 bg-gray-100 border-2 border-gray-200 focus:border-black focus:bg-white rounded-2xl text-sm font-medium resize-none outline-none transition-all placeholder:text-gray-400 disabled:opacity-50 max-h-32 overflow-y-auto"
          />
          <button
            onClick={() => sendMessage()}
