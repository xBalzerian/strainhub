      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* GMB Map */}
            {extra.gmb && (
              <div className="bg-white border-2 border-brand rounded-2xl overflow-hidden shadow-brutal">
                <div className="px-6 pt-5 pb-3 flex items-center justify-between">
                  ...
                  <a href={extra.gmb} target="_blank" rel="noopener noreferrer"
                    className="text-xs font-black text-lime-dark hover:underline flex items-center gap-1">
                    Open in Google Maps ↗
                  </a>
                </div>
                <div className="px-6 pb-2">
                  <p className="text-xs text-gray-500">{[sb.city, sb.state_province, sb.country].filter(Boolean).join(", ")}</p>
                </div>
                <iframe
                  src={`https://maps.google.com/maps?q=${encodeURIComponent([sb.name, sb.city, sb.state_province, sb.country].filter(Boolean).join(" "))}&output=embed&z=12`}
                  width="100%" height="280" loading="lazy" referrerPolicy="no-referrer-when-downgrade"
                  className="border-t-2 border-brand/20 w-full"
                  title={`${sb.name} location map`}
                />
              </div>
            )}
